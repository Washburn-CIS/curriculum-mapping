...
def load_standard(name, ay=None):
    standard = Standard(open(resource_filename('curriculumstandards', 'standards/'+name+'.xml')), name)
    if ay:
...
def load_standards():
    standards = {}
    for filename in next(walk(resource_filename('curriculumstandards', 'standards/')))[2]:
        standards[filename[0:-4]] = Standard(open(resource_filename('curriculumstandards', 'standards/' + filename)) ,filename[0:-4])
    return standards

def load_syllabus(ay, subject, number):
    """ loads the specified course (if it exists)"""
    return Syllabus(ay, resource_filename('curriculumstandards', 'syllabi/'+ay+'/'+subject+number+'.xml'))

def available_years():
    """ returns a list of academic-year (20XX-20YY) strings for all available years of curriculum definitions"""
    return list(listdir(resource_filename('curriculumstandards', 'syllabi/')))
